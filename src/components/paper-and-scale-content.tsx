...
interface PaperAndScaleContentProps {
...
export function PaperAndScaleContent({
  onOrientationChange,
  onPaperSizeChange,
  onScaleTypeChange,
  onScaleValueChange,
  ...
  const [internalPaperSizeSelect, setInternalPaperSizeSelect] = useState('item1'); // Default to A3

  // Sync internal state with props
  useEffect(() => {
    setInternalOrientation(orientation);
    setInternalPaperSize(paperSize);
    setInternalScaleType(scaleType);
    setInternalScaleValue(scaleValue);
    setInternalUnitsValue(unitsValue);
    setInternalScaleLineWeights(scaleLineWeights);
    setInternalFitToPaperSize(fitToPaperSize);
